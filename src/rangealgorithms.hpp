...
auto constexpr count_if(Container& c, UnaryPredicate const& p) {
    return std::count_if(std::begin(c), std::end(c), p);
}

template <typename Container, typename UnaryPredicate>
auto constexpr all_of(Container& c, UnaryPredicate const& p) {
    return std::all_of(std::begin(c), std::end(c), p);
}

template <typename Container, typename UnaryPredicate>
auto constexpr any_of(Container& c, UnaryPredicate const& p) {
