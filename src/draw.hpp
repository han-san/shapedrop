...
[[nodiscard]] auto inline to_screen_space(Rect<double> const square)
    -> Rect<double> {
  auto const dim = get_window_dimensions();
  return {square.x * dim.w, square.y * dim.h, square.w * dim.w,
          square.h * dim.h};
}

[[nodiscard]] auto inline to_normalized(Rect<double> const square)
    -> Rect<double> {
  auto const dim = get_window_dimensions();
  return {square.x / dim.w, square.y / dim.h, square.w / dim.w,
          square.h / dim.h};
...
[[nodiscard]] auto inline to_screen_space_height(double const height)
    -> double {
  return height * get_window_dimensions().h;
}
