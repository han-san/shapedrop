...
[[nodiscard]] auto static get_font_path() -> std::string { return "./"; }
...
auto init_font(std::string const& fontName) -> bool {
  ...
  stbtt_BakeFontBitmap(ttf_buffer, offset, fontHeight,
                       bakedCharsBitmap.bitmap.data(), bakedCharsBitmap.w,
                       bakedCharsBitmap.h, firstChar, charCount,
                       bakedChars.data());

  return true;
}

[[nodiscard]] auto static get_codepoint_kern_advance(char const codepoint,
  ...
  return scale *
         static_cast<double>(advance + stbtt_GetCodepointKernAdvance(
                                           &font, codepoint, nextCodepoint));
}

FontCharacter::FontCharacter(char const c, double const pixelHeight,
                             char const nextChar)
    : character(c), scale(static_cast<double>(stbtt_ScaleForPixelHeight(
                        &font, static_cast<float>(pixelHeight)))),
      advance(get_codepoint_kern_advance(c, nextChar, scale)) {
  bitmap = stbtt_GetCodepointBitmap(&font, 0, static_cast<float>(scale), c,
                                    &dimensions.w, &dimensions.h, &xoff, &yoff);
...
auto get_baked_chars() -> BakedChars const& { return bakedChars; }

auto get_baked_chars_bitmap() -> BakedCharsBitmap const& {
  return bakedCharsBitmap;
}

FontString::FontString(std::string_view const string,
                       double const pixelHeight) {
  auto w = 0.;
...
auto FontString::get_text_width(std::string_view const text,
                                double const fontHeight) -> double {
  auto width = 0.;
  auto const scale = static_cast<double>(
      stbtt_ScaleForPixelHeight(&font, static_cast<float>(fontHeight)));
...
auto FontString::get_text_width_normalized(std::string_view const text,
                                           double const fontHeightNormalized)
    -> double {
  return get_text_width(text, get_window_dimensions().h * fontHeightNormalized);
}

auto FontString::from_width(std::string_view const string,
                            double const desiredPixelWidth) -> FontString {
...
auto FontString::from_width_normalized(std::string_view const string,
                                       double const desiredWidth)
    -> FontString {
  return from_width(string, get_window_dimensions().w * desiredWidth);
}

auto FontString::from_height(std::string_view const string,
                             double const desiredPixelHeight) -> FontString {
  return FontString(string, desiredPixelHeight);
}
auto FontString::from_height_normalized(std::string_view const string,
                                        double const desiredHeight)
    -> FontString {
