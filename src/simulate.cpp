  ...
  if (programState.levelType == ProgramState::LevelType::Game) {
    ...
    if (!gameState.paused) {
      ...
      if (programState.frameStartClock >
        ...
        if (!gameState.board.is_valid_move(gameState.currentShape,
                                           V2::down())) {
          ...
          for (auto const position : shapePositions) {
            ...
            gsl::index index {position.y * gameState.board.columns +
                              position.x};
            gsl::at(gameState.board.data,
                    index) = {gameState.currentShape.color, true};
          }

          auto const tspin =
              gameState.currentRotationType
                  ? gameState.board.check_for_tspin(
                        gameState.currentShape, *gameState.currentRotationType)
                  : std::nullopt;
          ...
          case ClearType::Tspin_mini_double: {
            ...
            auto const comboScore =
                50 * gameState.comboCounter * gameState.level;
          ...
          gameState.level =
              gameState.linesCleared / 10 + gameState.startingLevel;
